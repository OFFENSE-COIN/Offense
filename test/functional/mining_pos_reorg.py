...
class ReorgStakeTest(Offense_CoinTestFramework):
    ...
    def check_money_supply(self, expected_ofc):
        # verify that nodes have the expected OFC supply
        ofc_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(ofc_supply, [DecimalAmt(expected_ofc)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that OFC supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check OFC supply...")
