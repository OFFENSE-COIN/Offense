UNIX BUILD NOTES
====================
Some notes on how to build OFFENSE_COIN in Unix.

Note
---------------------
Always use absolute paths to configure and compile OFFENSE_COIN and the dependencies,
...
This will build offense_coin-qt as well, if the dependencies are met.
...
memory available when compiling OFFENSE_COIN. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~offense_coin/+archive/offense_Coin).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:offense_Coin/offense_Coin
    sudo apt-get update
...
To build OFFENSE_COIN without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build offense_coin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a offense_coin-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip offense_coind" to strip the debug
...
Security
--------
To help make your OFFENSE_COIN installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, OFFENSE_COIN should be built with a non-executable stack
