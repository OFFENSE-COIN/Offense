...
*Setup instructions for a gitian build of OFFENSE_COIN using a VM or physical system.*

Gitian is the deterministic build process that is used to build the OFFENSE_COIN
...
to the OFFENSE_COIN GitHub release page.
...
- [Building OFFENSE_COIN](#building-offense_coin)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/offense_coin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/offense_coin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/offense_coin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/offense_coin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/offense_coin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/offense_coin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/offense_coin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@offense_coin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/offense_coin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/offense_coin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/offense_coin-osx-*-build.assert
```
