...
OFFENSE_COIN uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play OFCs (tOFC)" on the test network, if you
...
OFFENSE_COIN is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_offense_coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_offense_coin --log_level=test_suite
$ valgrind -v --leak-check=full src/offense_coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_offense_coin.coverage/index.html`.
```

**Sanitizers**
OFFENSE_COIN can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates OFCs (if wallet is enabled).
...
However, in open source software such as OFFENSE_COIN, where everyone uses
...
pay attention to for reviewers of OFFENSE_COIN code.

General OFFENSE_COIN
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: OFFENSE_COIN uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against OFFENSE_COIN so that they can be integrated
...
file descriptors it will cause problems with OFFENSE_COIN's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `offense_coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `offense_coin-cli` and the GUI debug console use this table to determine how to
