...
COfcStake* COfcStake::NewOfcStake(const CTxIn& txin, int nHeight, uint32_t nTime)
{
    ...
    if (!coin.IsSpent()) {
        ...
        // All good
        return new COfcStake(coin.out, txin.prevout, pindexFrom);
    }
    ...
    // All good
    return new COfcStake(txPrev->vout[txin.prevout.n], txin.prevout, pindexFrom);
}

bool COfcStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

CTxIn COfcStake::GetTxIn() const
{
    return CTxIn(outpointFrom.hash, outpointFrom.n);
}

CAmount COfcStake::GetValue() const
{
    return outputFrom.nValue;
}

bool COfcStake::CreateTxOuts(const CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal) const
{
...
CDataStream COfcStake::GetUniqueness() const
{
    //The unique identifier for a OFC stake is the outpoint
...
const CBlockIndex* COfcStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("COfcStake: uninitialized pindexFrom");
    return pindexFrom;
}
