...
void Shutdown()
{
    ...
    util::ThreadRename("offense_coin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf("Specify configuration file (default: %s)", OFFENSE_COIN_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", OFFENSE_COIN_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf("Disable all OFFENSE_COIN specific functionality (Masternodes, Budgeting) (0-1, default: %u)", 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf("Specify masternode configuration file (default: %s)", OFFENSE_COIN_MASTERNODE_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf("Set external address:port to get to this masternode (example: %s)", "128.127.106.235:18171"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2022 The Offense Coin Developers"))) + "\n" +
           "\n" +
...
{
    util::ThreadRename("offense_coin-loadblk");
    CImportingNow imp;
...
/** Sanity checks
 *  Ensure that OFFENSE_COIN is running in a usable environment with all
...
#endif
    LogPrintf("OFFENSE_COIN version %s\n", version_string);
}
...
{
    // Make sure only a single OFFENSE_COIN process is using the data directory.
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", OFFENSE_COIN_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if OFFENSE_COIN is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if OFFENSE_COIN is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //OFFENSE_COIN specific: spork DB's
                ...
                // OFFENSE_COIN: load previous sessions sporks if we have them.
...
// ********************************************************* Step 8: Backup and Load wallet
    ...
    if ((fMasterNode || masternodeConfig.getCount() > -1) && fTxIndex == false) {
        return UIError(strprintf(_("Enabling Masternode support requires turning on transaction indexing."
                                   "Please add %s to your configuration and start with %s"),
            "txindex=1", "-reindex"));
    }
