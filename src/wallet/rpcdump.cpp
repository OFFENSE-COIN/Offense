...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OFFENSE_COIN address or script");
        }
    }
    if (fRescan) {
        pwallet->RescanFromTime(TIMESTAMP_MIN, reserver, true /* update */);
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    bool fGood = true;
    {
        ...
        while (file.good()) {
            ...
            // Sapling keys
            // Let's see if the address is a valid OFFENSE_COIN spending key
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The offense_coin address for the private key\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by OFFENSE_COIN %s\n", CLIENT_BUILD);
                ...
                if (scannedTime <= GetImportTimestamp(request, now) || results.at(i).exists("error")) {
                    ...
                    result.pushKV("error", JSONRPCError(RPC_MISC_ERROR,
                                                strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                                          ...
                                                          "caused by pruning or data corruption (see offense_coind log for details) and could "
...
UniValue bip38encrypt(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"      (string, required) The offense_coin address for the private key (you must hold the key already)\n"
