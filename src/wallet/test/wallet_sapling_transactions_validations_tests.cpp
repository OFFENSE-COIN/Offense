...
BOOST_AUTO_TEST_CASE(test_in_block_and_mempool_notes_double_spend)
{
    ...
    // Now that we have the chain, let's shield 100 OFCs
    // single recipient
    std::vector<SendManyRecipient> recipients;
    ...
    SyncWithValidationInterfaceQueue();
    BOOST_CHECK_EQUAL(pwalletMain->GetAvailableShieldedBalance(), CAmount(100 * COIN)); // 100 shield OFCs
    BOOST_CHECK_EQUAL(pwalletMain->GetUnconfirmedShieldedBalance(), CAmount(0)); // 0 shield OFCs
