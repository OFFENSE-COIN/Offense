...
/**
 * Fees smaller than this (in uofc) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex, std::string& strError)
{
    ...
    if (txOut.nValue != Params().GetConsensus().nMNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s OFC", FormatMoney(Params().GetConsensus().nMNCollateralAmt));
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-offense_coin-address
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        COfcStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
...
CWallet* CWallet::CreateWalletFromFile(const std::string& name, const fs::path& path)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet) ||
        ...
        if (prev_version <= FEATURE_PRE_OFFENSE_COIN && walletInstance->IsLocked()) {
    ...
    if (fFirstRun) {
        if (!fLegacyWallet) {
            ...
            walletInstance->SetMaxVersion(FEATURE_PRE_OFFENSE_COIN);
        }
