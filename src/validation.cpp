...
#if defined(NDEBUG)
#error "OFFENSE_COIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uofc) are considered zero fee (for relaying, mining and transaction creation)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k OFC
    ...
    else if (nHeight >= nLast && nHeight <= 245000200)
    {
        return 2 * COIN;
    } 
    else 
...
{
    util::ThreadRename("offense_coin-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if (!IsBlockValueValid(pindex->nHeight, nExpectedMint, nMint, nBudgetAmt)) {
        return state.DoS(100, error("%s: reward pays too much (actual=%s vs limit=%s)", __func__, FormatMoney(nMint), FormatMoney(nExpectedMint)),
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Offense_Coin Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
 ...
 */
static bool CheckInBlockDoubleSpends(const CBlock& block, int nHeight, CValidationState& state, std::unordered_set<COutPoint, SaltedOutpointHasher>& spent_outpoints)
{
    const Consensus::Params& consensus = Params().GetConsensus();
