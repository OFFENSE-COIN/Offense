...
const char* const OFFENSE_COIN_CONF_FILENAME = "offensecoin.conf";
const char* const OFFENSE_COIN_PID_FILENAME = "offensecoin.pid";
const char* const OFFENSE_COIN_MASTERNODE_CONF_FILENAME = "masternode.conf";


// OFFENSE_COIN only features
// Masternode
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "offense_coind -foo=bar
...
#else
    const char* pszModule = "offense_coin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\OFFENSE_COIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\OFFENSE_COIN
// Mac: ~/Library/Application Support/OFFENSE_COIN
// Unix: ~/.offense_coin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OffenseCoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\OFFENSECOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\OFFENSECOINParams
    // Mac: ~/Library/Application Support/OFFENSECOINParams
    // Unix: ~/.OffenseCoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "OffenseCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectories(pathRet);
    return pathRet / "OffenseCoinParams";
#else
    // Unix
    return pathRet / ".offensecoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/offense_coin/sapling-spend.params";
        sapling_output = "/usr/share/offense_coin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/offense_coin/sapling-spend.params";
            sapling_output = "/usr/local/share/offense_coin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", OFFENSE_COIN_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()) {
        // Create empty offensecoin.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(confPath), "a");
        if (configFile != NULL) {
            std::string strHeader = "rpcuser=ofcuser\n"
                                    "rpcpassword=ofcpassword\n"
                                    "rpcport=18172\n"
                                    "port=18171\n"
                                    "server=1\n"
                                    "daemon=1\n"
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", OFFENSE_COIN_PID_FILENAME));
