...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("OFFENSE COINMiner : generated block is stale");
    }
    ...
    if (!res || sc.stateErrorFound()) {
        return error("OFFENSE COINMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("OFFENSE COINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("offense_coin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running OFFENSE COINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("OFFENSE COINMiner exception");
    } catch (...) {
        LogPrintf("OFFENSE COINMiner exception");
    }

    LogPrintf("OFFENSE COINMiner exiting\n");
}
