...
#include "qt/offense_coin/forms/ui_addresseswidget.h"
#include "qt/offense_coin/addressfilterproxymodel.h"
#include "qt/offense_coin/addresslabelrow.h"
#include "qt/offense_coin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(OFFENSE_COINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto ofcAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(ofcAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(ofcAdd);
        if (walletModel->updateAddressBookLabels(ofcAdd, label.toUtf8().constData(),
