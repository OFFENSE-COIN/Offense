...
#include "qt/offense_coin/forms/ui_masternodeswidget.h"

#include "qt/offense_coin/masternodewizarddialog.h"
#include "qt/offense_coin/mninfodialog.h"
#include "qt/offense_coin/mnrow.h"
...
MasterNodesWidget::MasterNodesWidget(OFFENSE_COINGUI* parent) : PWidget(parent),
    ...
    if (fs::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                       ...
                       "# Example: mn1 127.0.0.2:18171 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked()
{
    ...
    if (walletModel->getBalance() <= mnCollateralAmount) {
        inform(tr("Not enough balance to create a masternode, %1 required.")
                   .arg(GUIUtil::formatBalance(mnCollateralAmount, BitcoinUnits::OFC)));
        return;
    }
    showHideOp(true);
