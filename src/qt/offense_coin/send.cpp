...
#include "qt/offense_coin/send.h"
#include "qt/offense_coin/forms/ui_send.h"
#include "qt/offense_coin/addnewcontactdialog.h"
#include "qt/offense_coin/qtutils.h"
#include "qt/offense_coin/sendchangeaddressdialog.h"
#include "qt/offense_coin/optionbutton.h"
#include "qt/offense_coin/sendconfirmdialog.h"
#include "qt/offense_coin/guitransactionsutils.h"
...
SendWidget::SendWidget(OFFENSE_COINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a OFFENSE COIN URI"));
    ...
    setCssProperty(coinIcon, "coin-icon-ofc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onOFCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onOFCSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent OFCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield OFCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent OFCs to shield."));
    }
}
...
void SendWidget::onOFCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto ofcAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(ofcAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(ofcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ofcAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
