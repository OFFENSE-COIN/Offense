...
#include "qt/offense_coin/forms/ui_snackbar.h"
#include "qt/offense_coin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(OFFENSE_COINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &OFFENSE_COINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
