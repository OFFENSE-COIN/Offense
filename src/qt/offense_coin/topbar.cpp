...
#include "qt/offense_coin/topbar.h"
#include "qt/offense_coin/forms/ui_topbar.h"
#include "qt/offense_coin/lockunlock.h"
#include "qt/offense_coin/qtutils.h"
#include "qt/offense_coin/receivedialog.h"
#include "qt/offense_coin/loadingdialog.h"
#include "askpassphrasedialog.h"

#include "bitcoinunits.h"
#include "qt/offense_coin/balancebubble.h"
...
TopBar::TopBar(OFFENSE_COINGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopOfc, ui->labelAmountTopShieldedOfc}, "amount-small-topbar");
    setCssProperty({ui->labelAmountOfc}, "amount-topbar");
    setCssProperty({ui->labelPendingOfc, ui->labelImmatureOfc}, "amount-small-topbar");
    ...
    ui->pushButtonConf->setButtonText("offensecoin.conf");
...
void TopBar::onBtnConfClicked()
{
    ...
    if (!GUIUtil::openConfigfile())
        inform(tr("Unable to open offensecoin.conf with default application"));
}
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("OFFENSE_COIN")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // OFC Total
    QString totalOfc = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);
    ...
    // OFC
    // Top
    ui->labelAmountTopOfc->setText(totalTransparent);
    ui->labelAmountTopShieldedOfc->setText(totalShielded);
    // Expanded
    ui->labelAmountOfc->setText(totalOfc);
    ui->labelPendingOfc->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance + newBalance.unconfirmed_shielded_balance, nDisplayUnit));
    ui->labelImmatureOfc->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
