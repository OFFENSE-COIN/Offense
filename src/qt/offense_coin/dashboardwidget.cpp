...
#include "qt/offense_coin/dashboardwidget.h"
#include "qt/offense_coin/forms/ui_dashboardwidget.h"
#include "qt/offense_coin/sendconfirmdialog.h"
#include "qt/offense_coin/txrow.h"
...
DashboardWidget::DashboardWidget(OFFENSE_COINGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareOfc, "square-chart-ofc");
    setCssProperty(ui->labelSquareMN, "square-chart-mn");
    setCssProperty(ui->labelOfc, "text-chart-ofc");
    ...
    setCssProperty(ui->labelAmountOfc, "text-stake-ofc-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &OFFENSE_COINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("OFC")
    updateDisplayUnit();
}
...
QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isOfc = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair OFC, MN Reward
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ofc = (pair.first != 0) ? pair.first / 100000000 : 0;
            mn = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalOfc += pair.first;
            chartData->totalMN += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesOfc.append(ofc);
        chartData->valuesMN.append(mn);

        int max = std::max(ofc, mn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(31, 109, 180));
    set1->setColor(QColor(135, 198, 255));
    ...
    set0->append(chartData->valuesOfc);
    set1->append(chartData->valuesMN);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalOfc > 0 || chartData->totalMN > 0) {
        setCssProperty(ui->labelAmountOfc, "text-stake-ofc");
        setCssProperty(ui->labelAmountMN, "text-stake-mn");
    } else {
        setCssProperty(ui->labelAmountOfc, "text-stake-ofc-disable");
        setCssProperty(ui->labelAmountMN, "text-stake-mn-disable");
    }
    forceUpdateStyle({ui->labelAmountOfc, ui->labelAmountMN});
    ui->labelAmountOfc->setText(GUIUtil::formatBalance(chartData->totalOfc, nDisplayUnit));
