...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::OFC);
    ...
    if (settings.contains("nAnonymizeOffense_CoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizeoffense_coinamount", settings.value("nAnonymizeOffense_CoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in offense_coin.cpp during shotdown to also remove the window positions
