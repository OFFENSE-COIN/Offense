...
#define URI_SCHEME "offense_coin"
...
QString formatBalance(CAmount amount, int nDisplayUnit, bool Zofc)
{
...
QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool Zofc)
{
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter OFFENSE_COIN address (e.g. %1)").arg("O7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no OFFENSE_COIN: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::OFC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert offense_coin:// to offense_coin:
    //
    //    Cannot handle this later, because offense_coin:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::OFC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openConfigfile()
{
    return openFile(GetConfigFile(gArgs.GetArg("-conf", OFFENSE_COIN_CONF_FILENAME)), true);
}
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "OFFENSE_COIN (testnet).lnk";
    else if (chain == CBaseChainParams::REGTEST)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "OFFENSE_COIN (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "OFFENSE_COIN.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for OFFENSE_COIN*.lnk
...
{
    return GetAutostartDir() / "offense_coin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a offense_coin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=OFFENSE_COIN (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=OFFENSE_COIN (regtest)\n";
        else
            optionFile << "Name=OFFENSE_COIN\n";
