...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select OFC Outputs to Spend" :
            "Select Shielded OFC to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::OFC;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show OFFENSE_COIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
