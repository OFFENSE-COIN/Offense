...
#include "qt/offense_coin/splash.h"
#include "qt/offense_coin/welcomecontentwidget.h"
...
#include "qt/offense_coin/mnmodel.h"
...
{
    return QCoreApplication::translate("offense_coin", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in offense_coin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in offense_coin.qrc)
...
/** Class encapsulating OFFENSE_COIN startup and shutdown.
...
/** Main OFFENSE_COIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (OFFENSE_COINGUI)
...
private:
    ...
    OFFENSE_COINGUI* window{nullptr};
...
{
    window = new OFFENSE_COINGUI(networkStyle, nullptr);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &OFFENSE_COINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &OFFENSE_COINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (!vpwallets.empty()) {
            ...
            window->addWallet(OFFENSE_COINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(OFFENSE_COINGUI::DEFAULT_WALLET);
        ...
        // OFFENSE_COIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &OFFENSE_COINGUI::handlePaymentRequest);
        connect(window, &OFFENSE_COINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(nullptr, "Runaway exception", QObject::tr("A fatal error occurred. OFFENSE_COIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(offense_coin_locale);
    Q_INIT_RESOURCE(offense_coin);
    ...
    /// 6. Determine availability of data and blocks directory and parse offensecoin.conf
    ...
    try {
        gArgs.ReadConfigFile(gArgs.GetArg("-conf", OFFENSE_COIN_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    // offense_coin: links repeatedly have their payment requests routed to this process:
